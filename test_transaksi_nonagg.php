...
try {
    ...
    $sql = "TRUNCATE TABLE test_transaksi_agg_low";
...
// ambil data series stock
     ...
     $sqlsignal = $conn1->prepare("SELECT test_sinyal_agg_low.id_sinyal, test_sinyal_agg_low.id_csv,
	  test_sinyal_agg_low.Date,
	  test_sinyal_agg_low.sinyal,
	  csv_test.open,
	  csv_test.High,
	  csv_test.Low,
	  csv_test.close
	  FROM test_sinyal_agg_low
	  INNER JOIN csv_test
	  ON test_sinyal_agg_low.Date = csv_test.date
	  ORDER BY test_sinyal_agg_low.Date");
     $sqlsignal->execute();
    ...
    $sqlt = $conn5->prepare("INSERT INTO test_transaksi_agg_low (id_sinyal,Tanggal,Sinyal,Uang) VALUES (:id_sinyal,:Tanggal,:Sinyal,:Uang)");
...
<div class="row">
	  ...
	  <h3>Transaksi BBTN Menggunakan Model Data C</h3>
      ...
      $sqlz = $conn4->prepare("SELECT Count(*) from test_transaksi_agg_low");
      $sqlz->execute();
    ...
    <table class="table table-hover vertical-align">
      ...
      $conn4 = new PDO('mysql:host='.$host.';dbname='.$database, $username, $password);
      $conn4->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $sqlv = $conn4->prepare("SELECT * from test_transaksi_agg_low");
      $sqlv->execute();
