...
try {
    ...
    $sql = "TRUNCATE TABLE test_transaksi_agg_norm";
...
// ambil data series stock
     ...
     $sqlsignal = $conn1->prepare("SELECT test_sinyal_agg_norm.id_sinyal, test_sinyal_agg_norm.id_csv,
	  test_sinyal_agg_norm.Date,
	  test_sinyal_agg_norm.sinyal,
	  csv_test.open,
	  csv_test.High,
	  csv_test.Low,
	  csv_test.close
	  FROM test_sinyal_agg_norm
	  INNER JOIN csv_test
	  ON test_sinyal_agg_norm.Date = csv_test.date
	  ORDER BY test_sinyal_agg_norm.Date");
     $sqlsignal->execute();
    ...
    $sqlt = $conn5->prepare("INSERT INTO test_transaksi_agg_norm (id_sinyal,Tanggal,Sinyal,Uang) VALUES (:id_sinyal,:Tanggal,:Sinyal,:Uang)");
...
<?php require 'header.php' ?>
     ...
     <div class="row">
	  ...
	  <h3>Transaksi BBTN Menggunakan Model Data B</h3>
      ...
      $sqlz = $conn4->prepare("SELECT Count(*) from test_transaksi_agg_norm");
      $sqlz->execute();
    ...
    <table class="table table-hover vertical-align">
      ...
      $conn4 = new PDO('mysql:host='.$host.';dbname='.$database, $username, $password);
      $conn4->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $sqlv = $conn4->prepare("SELECT * from test_transaksi_agg_norm");
      $sqlv->execute();
