...
try {
    ...
    $sql = "TRUNCATE TABLE test_transaksi_agg_low_norm";
...
// ambil data series stock
     ...
     $sqlsignal = $conn1->prepare("SELECT test_sinyal_agg_low_norm.id_sinyal, test_sinyal_agg_low_norm.id_csv,
    test_sinyal_agg_low_norm.Date,
    test_sinyal_agg_low_norm.sinyal,
    csv_test.open,
    csv_test.High,
    csv_test.Low,
    csv_test.close
    FROM test_sinyal_agg_low_norm
    INNER JOIN csv_test
    ON test_sinyal_agg_low_norm.Date = csv_test.date
    ORDER BY test_sinyal_agg_low_norm.Date");
     $sqlsignal->execute();
  ...
  // Menentukan sinyal buy or sell
    ...
    $sqlt = $conn5->prepare("INSERT INTO test_transaksi_agg_low_norm (id_sinyal,Tanggal,Sinyal,Uang) VALUES (:id_sinyal,:Tanggal,:Sinyal,:Uang)");
      ...
      <section id="three" class="wrapper">
        <div class="inner">
          ...
          <body>
           ...
           <button onclick="location.href = 'test_transaksi_agg.php';" id="myButton" class="float-left submit-button" >Model Data A</button>
...
<div class="row">
    ...
    <h3>Transaksi BBTN  Menggunakan Model Data D</h3>
      ...
      $sqlz = $conn4->prepare("SELECT Count(*) from test_transaksi_agg_low_norm");
      $sqlz->execute();
    ...
    <table class="table table-hover vertical-align">
      ...
      $conn4 = new PDO('mysql:host='.$host.';dbname='.$database, $username, $password);
      $conn4->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $sqlv = $conn4->prepare("SELECT * from test_transaksi_agg_low_norm");
      $sqlv->execute();
